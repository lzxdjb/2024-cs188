...
class ReflexAgent(Agent):
    ...
    def evaluationFunction(self, currentGameState: GameState, action):
        ...
        food_scores = [manhattanDistance(newPos, food) for food in newFood.asList()]

        if len(food_scores) != 0:  
            food_distance = min(food_scores) #Pacmen should eat the cloest point first!!

        elif len(food_scores) == 0:
            food_distance = 0

        # food_distance = min(food_scores)
        ...
        # if ghost_distance < 5 :
        #     return min(ghost_distance) - food_distance

        newScaredTimes = [ghostState.scaredTimer for ghostState in newGhostStates]  
        scared_time = min(newScaredTimes)

        # a , b , c , d  = - 200 , - 5  , 10 , 100
        a , b , c , d  = - 300 , - 5 , 0 , 100000

        

        return a * food_count + b * food_distance + c * ghost_distance + d * scared_time
...
def betterEvaluationFunction(currentGameState: GameState):
    ...
    powerPosition = currentGameState.getCapsules()
    # print(powerPosition)
    power_scores = [manhattanDistance(Position , newPos) for Position in powerPosition]
    power_distance = min(power_scores)
    # print(powerPulltet_position)
    ...
    if scared_time > 0:
        ...
        return a * food_count + b * food_distance 
        


    # return a * food_count + b * food_distance + c * ghost_distance + d * power_score
    return a * food_count + b * food_distance + c * ghost_distance 
...
better = betterEvaluationFunction
        ...
        wallRight = walls[len(walls) - 1][0]
        wallBottom = walls[len(walls) - 1][1]



        tpSize = 3
        tracerPoints = []
        for x in range(0, wallRight, tpSize):
            for y in range(0, wallBottom, tpSize):
                tracerPoints.append((x, y))
        
        tpScores = {}
        for tracerPoint in tracerPoints:
            tpFoodDistances = [manhattanDistance(tracerPoint, food) for food in newFood.asList()]
            tpFoodLessThan = [dist for dist in tpFoodDistances if dist <= tpSize]
            tpScores[tracerPoint] = len(tpFoodLessThan)
        

        tracerPointMetric = 0
        for tracerPoint in tracerPoints:
            distanceToTP = manhattanDistance(newPos, tracerPoint)
            if distanceToTP == 0:
                distanceToTP = 1
            tracerPointMetric +=  (1 / distanceToTP) + tpScores[tracerPoint]
